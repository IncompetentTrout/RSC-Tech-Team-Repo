...
public class SaveManager : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        {
            SceneManager.LoadScene(SceneManager.GetActiveScene().buildIndex); //reload the scene to re-run the "loadFromJson()" in awake
        }
    ...
    {
        string savDat = JsonUtility.ToJson(saveData); //saves the saveData class as a json formatted string
        string filePath = Application.persistentDataPath + "/PlayerData.json"; //creates/overwrites a PlayerData.json file (simplified explaination)
        Debug.Log(filePath);
    ...
    {
        string filepath = Application.persistentDataPath + "/PlayerData.json";   //gets the filepath to the save data file
        string savDat = System.IO.File.ReadAllText(filepath); //takes the file data into a string

        if(JsonUtility.FromJson<PlayerSettings>(savDat) != null) //if the .json isnt null
        {

            saveData = JsonUtility.FromJson<PlayerSettings>(savDat); //converts from .json to PlayerSettings class and saves in in the saveData
            if (!saveData.isNewGame) //if it is not a new game/cleared save
            {
                player.transform.position = checkPoints[saveData.lastSavedPosition].transform.position;  //sets the players position to the last checkpoint
            }
    ...
    {
        for(int i = 0; i < checkPoints.Length; i++) //uses a for loop to find what checkpoint was hit (so we can use the one we can use the index)
        {
...
public class PlayerSettings //class that contains the data for the player, needs to be stored as a publically available class outside of this script
{
