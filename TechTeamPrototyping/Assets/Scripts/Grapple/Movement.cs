...
public class Movement : MonoBehaviour {
    ...
    private void CheckInput() {
        ...
        else if (Input.GetKeyDown(KeyCode.S)) ChangeLayer(-1); //move layer towards camera (negative on z axis)
    }
    ...
    protected void ChangeLayer(int direction) //moves the player back or forth on the layer
    {
        var checkPos = new Vector3(gameObject.transform.position.x, gameObject.transform.position.y - 0.3f,
            gameObject.transform.position.z); //current player position with check closer to feet of player
        switch (direction) //checking if there is an object blocking where the desired layer move is
        {
            case -1:
                canShiftLayers =
                    !Physics.Raycast(checkPos, Vector3.back,
                        layerChangeDist); //if the raycast towards camera returns a hit, the player cannot shift
                break;
            case 1:
                canShiftLayers =
                    !Physics.Raycast(checkPos, Vector3.forward,
                        layerChangeDist); //if the raycast away from camera returns a hit, the player cannot shift
                break;
        }
        ...
        else if (canShiftLayers) //otherwise if they can shift
        {
            ...
            gameObject.transform.position =
                new Vector3(pos.x, pos.y,
                    pos.z + direction *
                    layerChangeDist); //move the player by the required distance in the right direction to be on the layer
        }
    }
