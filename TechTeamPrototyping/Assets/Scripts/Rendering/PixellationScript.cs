...
{
    [SerializeField]
    private int downScale; //amount the pixel resolution is divided by
    ...
    [SerializeField]
    private RawImage pixelEffect; //reference to the UI RawImage that renders the pixellation on screen
    ...
    {
        RectTransform effectSize = pixelEffect.GetComponent<RectTransform>(); //obtains the rectTransform of the effect renderer to change its size
        effectSize.sizeDelta = new Vector2((mainCamera.pixelWidth), (mainCamera.pixelHeight)); //changes the size of the pixelEffect to the size of the camera

        Vector2 cameraSize = new Vector2((mainCamera.pixelWidth) / downScale, (mainCamera.pixelHeight) / downScale); //creates a downscaled version of the main camera
